...
RUN apt-get update && apt-get install -y --no-install-recommends haskell-platform pkg-config && cabal new-update && cabal new-install cabal-install && cabal new-install --only-dependencies -j4
